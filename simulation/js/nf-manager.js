...
class NFManager {
    ...
    createNetworkFunction(type, position = null) {
        ...
        // SPECIAL CASE: UPF gets additional ogstun network interface
        if (type === 'UPF') {
            nf.config.ogstunInterface = {
                interfaceName: 'ogstun',
                network: '10.0.0.0/28',
                gatewayIP: '10.0.0.1',
            ...
            console.log(`🌐 UPF ${nf.name} created with ogstun interface: ${nf.config.ogstunInterface.gatewayIP} (${nf.config.ogstunInterface.network})`);
        }
        ...
        if (window.logEngine) {
            ...
            // Special logging for UPF ogstun interface
            if (nf.type === 'UPF' && nf.config.ogstunInterface) {
                window.logEngine.addLog(nf.id, 'INFO',
                    `ogstun network interface created: ${nf.config.ogstunInterface.network}`, {
                    interfaceName: nf.config.ogstunInterface.interfaceName,
                    network: nf.config.ogstunInterface.network,
                    gatewayIP: nf.config.ogstunInterface.gatewayIP,
     ...
     */
    registerUEAndEstablishPDU(ueId, upfId = null) {
        ...
        // Assign IP from ogstun network
        const assignedIP = this.assignOgstunIPToUE(upf, ue);
        if (!assignedIP) {
            console.error('❌ Failed to assign ogstun IP to UE:', ue.name);
            return false;
        }
        ...
        if (window.logEngine) {
            window.logEngine.addLog(ueId, 'SUCCESS',
                ...
                upfName: upf.name,
                upfId: upf.id,
                ogstunNetwork: upf.config.ogstunInterface.network,
                upfGateway: upf.config.ogstunInterface.gatewayIP,
    ...
    /**
     * Assign ogstun IP to UE from UPF's ogstun interface
     ...
     */
    assignOgstunIPToUE(upf, ue) {
        if (!upf.config.ogstunInterface) {
            console.error('❌ UPF does not have ogstun interface:', upf.name);
            return null;
        }

        const ogstun = upf.config.ogstunInterface;
        
        // Check if UE already has an IP assigned
        const existingAssignment = ogstun.assignedIPs.find(assignment => assignment.ueId === ue.id);
        if (existingAssignment) {
            console.log(`ℹ️ UE ${ue.name} already has ogstun IP: ${existingAssignment.ip}`);
        ...
        while (ogstun.nextAvailableIP <= 14) {
            const candidateIP = `10.0.0.${ogstun.nextAvailableIP}`;
            
            // Check if IP is already assigned
            const isAssigned = ogstun.assignedIPs.some(assignment => assignment.ip === candidateIP);
            
            if (!isAssigned) {
                // Assign IP to UE
                ogstun.assignedIPs.push({
                    ueId: ue.id,
                    ueName: ue.name,
                    ip: candidateIP,
                    assignedAt: Date.now()
                ...
                ogstun.nextAvailableIP++;
                
                // Update UPF in data store
                window.dataStore.updateNF(upf.id, upf);
                
                console.log(`🌐 Assigned ogstun IP ${candidateIP} to UE ${ue.name} via UPF ${upf.name}`);
                return candidateIP;
            }
        ...
        console.error('❌ No more ogstun IPs available in network 10.0.0.0/28');
        return null;
    }
